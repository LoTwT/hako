...
>
> 临时性：属于同一临时规格组，历史归档说明见[文档索引](../index.md#临时规格)。
...
- 原生 Bearer token 只交给 Rust identity service 并存入环境隔离的 Stronghold；Vue/WebView 只能取得脱敏会话状态。Hako auth adapter 通过 Better Auth Bearer 插件验证该 token；`AuthResponseBoundary` 保证 Web 登录及会话响应的 header 和 JSON body 都不暴露 JS 可读的 session secret。token 过期或撤销时，受保护的 Hako API 返回第 7.4 节定义的 401 `authentication_required`；客户端收到 401 后的处理以[客户端错误映射](./hako-client-foundation.md#84-调度与故障隔离)为准。
